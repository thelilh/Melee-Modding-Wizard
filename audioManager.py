...
def getHpsFile(windowParent=None, isoPath=''):
    ...
    if os.path.splitext(newFilePath)[1].lower() != '.hps':
        ...
        globalData.gui.updateProgramStatus(
            'Converting file to HPS format...', forceUpdate=True)
        command = '"{}" "{}" "{}"{}'.format(
            meleeMediaExe, newFilePath, outputPath, loopEditorWindow.loopArg)
        returnCode, output = cmdChannel(command)

        if returnCode != 0:
            globalData.gui.updateProgramStatus(
                'Conversion failed; {}'.format(output))
            msg('File conversion to HPS format failed; {}'.format(
    ...
    try:
        newFileObj = MusicFile(None, -1, -1, isoPath,
                               extPath=newFilePath, source='file')
        newFileObj.getData()
        return newFileObj
    except Exception as err:
        print('Exception during file initialization; {}'.format(err))
        globalData.gui.updateProgramStatus(
            'Unable to replace the file; {}'.format(err), error=True)
        return None
...
class AudioManager(ttk.Frame):
    ...
    def __init__(self, parent, mainGui):
        ...
        self.boldFont = tkFont.Font(
            weight='bold', size=mainGui.defaultFontSize)
        ...
        self.fileTree = NeoTreeview(self, columns=(
            'filename'), show='tree', yscrollcommand=treeScroller.set)
        self.fileTree.column('#0', width=300)
        self.fileTree.column('filename', width=140)
        self.fileTree.tag_configure(
            'playing', foreground='#4030ec', font=self.boldFont)  # Dark blue/Purple
        self.fileTree.tag_configure('changed', foreground='red')
        self.fileTree.tag_configure(
            'changesSaved', foreground='#292')  # The 'save' green color
        ...
        generalLabelFrame = ttk.LabelFrame(
            infoPane, text='  Disc Info  ', labelanchor='n', padding=(20, 0, 20, 4))
        ttk.Label(generalLabelFrame, text=(
        ...
        trackInfoLabelFrame = ttk.LabelFrame(
            infoPane, text='  Track Info  ', labelanchor='n', padding=(20, 0, 20, 4))
        ttk.Label(trackInfoLabelFrame, text=(
        ...
        ttk.Button(self.controlsFrame, text='Export', command=self.exportTrack,
                   state='disabled').grid(column=0, row=0, padx=5, pady=5)
        ttk.Button(self.controlsFrame, text='Import', command=self.importTrack,
                   state='disabled').grid(column=1, row=0, padx=5, pady=5)
        ttk.Button(self.controlsFrame, text='Rename', command=self.rename,
                   state='disabled').grid(column=0, row=1, padx=5, pady=5)
        ttk.Button(self.controlsFrame, text='Edit Loop', command=self.editLoop,
                   state='disabled').grid(column=1, row=1, padx=5, pady=5)
        ttk.Button(self.controlsFrame, text='Delete', command=self.delete,
                   state='disabled').grid(column=0, row=2, padx=5, pady=5)
        ttk.Button(self.controlsFrame, text='Add Track', command=self.addTrack).grid(
            column=1, row=2, padx=5, pady=5)
        ttk.Button(self.controlsFrame, text='Find References', command=self.findReferences,
                   state='disabled').grid(column=0, row=3, columnspan=2, ipadx=10, padx=5, pady=5)
        ...
        ttk.Button(infoPane, text='Color Code by File Size',
                   command=self.colorCode).pack(pady=(0, 6), ipadx=10)
        ...
        # The above should also happen first by default
        self.fileTree.bind('<Double-1>', self.controlModule.playAudio)
        ...
        for musicFile in globalData.disc.files.itervalues():
            ...
            if musicFile.isHexTrack:
                if not self.fileTree.exists('hextracks'):
                    self.fileTree.insert('', 'end', iid='hextracks', text=' Hex Tracks  (20XX Custom Music)', values=(
                        '', 'cFolder'), image=globalData.gui.imageBank('musicIcon'))
                parent = 'hextracks'
            elif musicFile.filename.startswith('ff_'):
                if not self.fileTree.exists('fanfare'):
                    self.fileTree.insert('', 'end', iid='fanfare', text=' Fanfare  (Victory Audio Clips)', values=(
                        '', 'cFolder'), image=globalData.gui.imageBank('audioIcon'))
                parent = 'fanfare'
            else:
                parent = ''

            # Add the musicFile to the treeview
            self.fileTree.insert(parent, 'end', iid=musicFile.isoPath,
                                 text=musicFile.shortDescription, values=(musicFile.filename, 'file'))
            # if musicFile.isHexTrack:
    ...
    def getSelectedFile(self):
        ...
        # This was probably a folder that was clicked on (being in the fileTree list means the file must be in the disc)
        if not musicFile:
    ...
    def updateGeneralInfo(self, filecount=0, totalMusicSize=0):
        ...
        totalDiscFilesize = globalData.disc.getDiscSizeCalculations(ignorePadding=True)[
            0]
        spaceRemaining = FileSystem.disc.defaultGameCubeMediaSize - totalDiscFilesize

        self.generalInfoLabel['text'] = '\n'.join([str(filecount), humansize(
            totalDiscFilesize), humansize(totalMusicSize), humansize(spaceRemaining)])
    ...
    def onFileTreeSelect(self, event=None):
        ...
        globalData.gui.updateProgramStatus(
            '{} selected'.format(musicFile.shortDescription))
        ...
        if musicFile.isHexTrack and musicFile.trackNumber <= 0x30 and musicFile.trackNumber != 0:
            musicId = '0x{:X} | 0x{:X}'.format(
                musicFile.musicId, musicFile.trackNumber | 0x10000)
        else:
            musicId = uHex(musicFile.musicId)

        # Format file size and track duration strings
        fileSizeString = '{}   (0x{:X})'.format(humansize(
            musicFile.size), musicFile.size)  # Displays as MB/KB, and full hex value
        durationString = self.formatDuration(musicFile.duration)
    ...
    def exportTrack(self):
        ...
        if not musicFile:  # Failsafe; the export button should be disabled
            if not self.fileTree.selection():
                msg("You must first select a file that you'd like to export.",
                    'No file selected', warning=True)
            else:
                # todo: add multi-file export
                msg('Please select just one file to export.',
                    'Invalid selection', warning=True)
            return

        # Determine the filetype to try to save as by default
        if self.lastExportFormat == 'hps':
            defaultName = musicFile.filename
            fileTypeOptions = [("HPS files", '*.hps'),
                               ('WAV files', '*.wav'), ("All files", "*.*")]
        else:
            defaultName = musicFile.filename[:-3] + 'wav'
            fileTypeOptions = [('WAV files', '*.wav'),
                               ("HPS files", '*.hps'), ("All files", "*.*")]
        ...
        if savePath.lower().endswith('.wav'):
            ...
            try:
                ...
                if wavFilePath:
                    ...
                    # Will attempt to delete the file after the move
                ...
                msg('Unable to convert the file to WAV format; {}.\n\nYou may still be able to export it as an HPS file.'.format(
                    err), 'Unable to Export', error=True)
        else:
            self.lastExportFormat = 'hps'
        ...
        if successful:
            globalData.gui.updateProgramStatus(
                'File exported successfully', success=True)
        else:
    ...
    def importTrack(self):
        ...
        if not musicFile:  # Failsafe; the export button should be disabled
            if not self.fileTree.selection():
                ...
                msg('Please select just one file to replace.',
                    'Invalid selection', warning=True)
            return
        ...
        if newMusicFile:
            ...
            globalData.gui.updateProgramStatus(
                'File Replaced. Awaiting Save', success=True)
    ...
    def rename(self):
        ...
        if musicFile.isHexTrack:
            ...
            cssFile = globalData.disc.files.get(
                globalData.disc.gameId + '/MnSlChr.0sd')
            if not cssFile:
                msg("Unable to update CSS with song name; the CSS file (MnSlChr.0sd) could not be found in the disc.")
                globalData.gui.updateProgramStatus(
                    "Unable to update CSS with song name; couldn't find the CSS file in the disc", error=True)
                return
            ...
            if charLimit == -1:
                ...
                globalData.gui.updateProgramStatus(
                    "Unable to update CSS with song name; a character limit could not be determined", error=True)
                return
        else:
            charLimit = 42  # Arbitrary; don't want it too long though, for readability

        # Prompt the user to enter a new name
        newName = getNewNameFromUser(
            charLimit, message='Enter a new name:', defaultText=musicFile.shortDescription)
        if not newName:
        ...
        if returnCode == 0:
            ...
            if globalData.gui.discTab:
                globalData.gui.discTab.updateDescription(
                    musicFile.isoPath, newName)

            self.fileTree.item(musicFile.isoPath, text=newName)

            if musicFile.isHexTrack:
                globalData.gui.updateProgramStatus(
                    'Song name updated in the CSS file', success=True)
            else:
                globalData.gui.updateProgramStatus('File name updated in the {}.yaml config file'.format(
                    globalData.disc.gameId), success=True)
        elif returnCode == 1:
            globalData.gui.updateProgramStatus(
                'Unable to update name/description in the {}.yaml config file'.format(globalData.disc.gameId), error=True)
        elif returnCode == 2:
            globalData.gui.updateProgramStatus(
                "Unable to update CSS with the name; couldn't find the CSS file in the disc", error=True)
        elif returnCode == 3:
            globalData.gui.updateProgramStatus(
                "Unable to update CSS with the name; couldn't save the name to the CSS file", error=True)
        else:
    ...
    def delete(self):
        ...
        for iid in folderIids:
            try:
                self.fileTree.delete(iid)
            except:
                pass  # May have already been removed alongside a parent folder
        ...
        detailsTab = globalData.gui.discDetailsTab
        if detailsTab:
            detailsTab.isoFileCountText.set(
                "{:,}".format(len(globalData.disc.files)))
            # detailsTab # todo: disc size as well
        ...
        if len(fileObjects) == 1:
            ...
            globalData.gui.updateProgramStatus(
                '{} files removed from the disc'.format(len(fileObjects)))
    ...
    def addTrack(self):
        ...
        if globalData.disc.is20XX:
            ...
            # Auto-generated would be 'GALE01/audio/us/1padv.ssm'
            insertionKey = globalData.disc.gameId + '/audio/1padv.ssm'
            insertAfter = False

            # Check for an unused hex track, and set file name suggestions
            for i in range(1, 0x100):
                # Generates a name like 'GALE01/audio/01.hps'
                isoPath = '{}/audio/{:02X}.hps'.format(gameId, i)
            ...
            unusedHexTrackName = ''
            # Auto-generated would be 'GALE01/audio/us/1padv.ssm'
            insertionKey = globalData.disc.gameId + '/audio/zs.ssm'
            insertAfter = True
        ...
        if globalData.gui.discTab:
            globalData.gui.discTab.loadDisc(
                updateStatus=False, preserveTreeState=True)
            globalData.gui.discTab.isoFileTree.item(
                newTrack.isoPath, tags='changed')
    ...
    def findReferences(self):
        ...
        for fileObj in gameFiles.itervalues():
            ...
            for i in range(1, musicTable.entryCount):
                ...
                # Getting values 1 through 4 of the entry
                secondaryRefs.extend(
                    values[i*valuesPerEntry+1:i*valuesPerEntry+5])
        ...
        primaryReferences = [u'{} ({})'.format(
            fileObj.filename, fileObj.longDescription) for fileObj in primaryReferences]
        secondaryReferences = [u'{} ({})'.format(
            fileObj.filename, fileObj.longDescription) for fileObj in secondaryReferences]
        ...
        if not primaryReferences and not secondaryReferences:
            ...
            if primaryReferences:
                self.referencesList['text'] = 'Only found as a primary reference with ' + \
                    primaryReferences[0]
            else:
                self.referencesList['text'] = 'Only found as a secondary reference with ' + \
                    secondaryReferences[0]
        else:
            self.referencesList['text'] = ''
            if primaryReferences:
    ...
    def colorCode(self):
        ...
        details = 'Smallest track:    {}   ({})'.format(
            humansize(minSize), minTrackName)
        details += '\nLargest track:    {}   ({})'.format(
            humansize(maxSize), maxTrackName)
        details += '\nAverage track size:    ' + \
            humansize(totalTrackSize / totalFiles)
        ...
        for iid in fileIids:
            ...
            percentOfRange = float(
                fileObj.size - minSize) / (maxSize - minSize)
            newColor = []
...
class LoopEditorWindow(BasicWindow):
    ...
    def __init__(self, title='Loop Editor'):
        ...
        # Ok / Cancel buttons
        ttk.Button(self.window, text='Ok', command=self.submit).grid(
            column=0, row=1, ipadx=0, padx=6, pady=(4, 14))
        ttk.Button(self.window, text='Cancel', command=self.cancel).grid(
            column=1, row=1, padx=6, pady=(4, 14))
        ...
        # Freezes the GUI until this window is closed.
    ...
    def submit(self):
        ...
        try:
            ...
            msg('Invalid input for a custom loop; {}'.format(err),
                'Invalid Loop Input', self.window, error=True)
            return
...
class LoopEditor(ttk.Frame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        ttk.Radiobutton(self, text='None', variable=self.loopTrack, value=0,
                        command=self.toggleTrackLooping).grid(column=1, row=0, padx=6, pady=4)
        ttk.Radiobutton(self, text='Normal', variable=self.loopTrack, value=1,
                        command=self.toggleTrackLooping).grid(column=2, row=0, padx=6, pady=4)
        ttk.Radiobutton(self, text='Custom', variable=self.loopTrack, value=2,
                        command=self.toggleTrackLooping).grid(column=3, row=0, padx=6, pady=4)
    ...
    def minute(self):
        ...
        if minuteText:
            ...
            if minute < 0 or minute > 60:
                raise Exception(
                    'minute data entry is out of bounds. Should be between 0 and 60.')
        else:
            minute = 0
    ...
    def second(self):
        ...
        if secondString:
            ...
            if second < 0 or second > 60:
                raise Exception(
                    'second data entry is out of bounds. Should be a float between 0 and 60.')
        else:
            second = 0.0
    ...
    def loopArg(self):
        ...
        if loopType == 2:  # Custom loop (to a specific minute/second)
            # {:09.6f} pads left up to 9 characters, with last 6 for decimal places
            loopArg = ' -loop 00:{:02}:{:09.6f}'.format(
...
class TrackAdder(BasicWindow):
    ...
    def __init__(self, defaultFilename, defaultHexTrackName):
        ...
        # File chooser
        ttk.Button(self.window, text='Choose file', command=self.chooseTrackFile).grid(
            column=0, columnspan=2, row=0, padx=6, pady=(14, 4))
        self.filenameDisplay = ttk.Label(self.window, text='', wraplength=500)
        self.filenameDisplay.grid(
            column=0, columnspan=2, row=1, padx=6, pady=4)
        ttk.Separator(self.window, orient='horizontal').grid(
            column=0, columnspan=2, row=2, sticky='ew', padx=50, pady=4)
        ...
        if defaultHexTrackName:
            ...
            ttk.Label(hexTrackDecisionFrame, text='Add as Hex Track:').grid(
                column=0, row=0, padx=6)
            ttk.Radiobutton(hexTrackDecisionFrame, text='Yes', variable=self.addAsHexTrack,
                            value=True, command=self.populateNameInputFrame).grid(column=1, row=0, padx=6)
            ttk.Radiobutton(hexTrackDecisionFrame, text='No', variable=self.addAsHexTrack,
                            value=False, command=self.populateNameInputFrame).grid(column=2, row=0, padx=6)
        ...
        ttk.Separator(self.window, orient='horizontal').grid(
            column=0, columnspan=2, row=5, sticky='ew', padx=50, pady=4)
        ...
        ttk.Separator(self.window, orient='horizontal').grid(
            column=0, columnspan=2, row=7, sticky='ew', padx=50, pady=4)

        # Track name (nickname) entry
        nicknameEntryFrame = ttk.Frame(self.window)
        ttk.Label(nicknameEntryFrame, text='Track Name:').grid(
            column=0, row=0, padx=6, pady=4)
        validationCommand = globalData.gui.root.register(self.nicknameModified)
        self.nicknameEntry = ttk.Entry(
            nicknameEntryFrame, width=33, validate='key', validatecommand=(validationCommand, '%P'))
        ...
        ttk.Button(self.window, text='Confirm', command=self.submitFile).grid(
            column=0, row=9, ipadx=0, padx=6, pady=(4, 14))
        ttk.Button(self.window, text='Cancel', command=self.cancel).grid(
            column=1, row=9, padx=6, pady=(4, 14))
    ...
    def determineMaxNickLength(self):
        ...
        if self.addAsHexTrack.get():
            if self.hexTrackMaxNickLen == -1:
                trackNumber = int(os.path.splitext(
                    self.defaultHexTrackName)[0], 16)
                cssFile = globalData.disc.files.get(
                    globalData.disc.gameId + '/MnSlChr.0sd')
                self.hexTrackMaxNickLen = cssFile.checkMaxHexTrackNameLen(
                    trackNumber)
    ...
    def nicknameModified(self, newString):
        ...
        self.nickLengthText['text'] = '{}/{}'.format(
            newStrLen, self.nickMaxLength)
    ...
    def populateNameInputFrame(self):
        ...
        if self.addAsHexTrack.get():
            ...
            ttk.Label(self.nameInputFrame, text='First empty Hex Track slot (must be added in order):').grid(
                column=0, row=0, padx=6, pady=4)
            self.filenameLabel = ttk.Label(
                self.nameInputFrame, text=self.defaultHexTrackName)
            self.filenameLabel.grid(column=0, row=1, padx=6, pady=4)
        else:
            ttk.Label(self.nameInputFrame, text='Enter a disc file name less than 30 characters:').grid(
                column=0, columnspan=2, row=0, padx=6, pady=4)
            self.filenameLabel = ttk.Label(
                self.nameInputFrame, text=self.defaultName)
            self.filenameLabel.grid(column=0, row=1, padx=6, pady=4)

            ttk.Button(self.nameInputFrame, text='Change', command=self.promptForDiscFilename).grid(
                column=1, row=1, padx=6, pady=4)
    ...
    def promptForDiscFilename(self):
        ...
        if hexTrack:
            charLimit = 6
            newName = getNewNameFromUser(
                charLimit, illegalChars, defaultText=self.defaultHexTrackName)
        else:
            charLimit = 30  # todo: invesigate. notes imply this, but what's the source?
            newName = getNewNameFromUser(
                charLimit, illegalChars, defaultText=self.defaultName)
        ...
        if not newName:
            ...
            if len(newName) > charLimit and not hexTrack:
                msg('The name is too long after adding the .hps file extension. Please choose a shorter name.',
                    'Filename too long', self.window)
                return
    ...
    def submitFile(self):
        ...
        isoPath = '{}/audio/{}'.format(globalData.disc.gameId,
        ...
        if isoFile:
            ...
            msg('No external file has been chosen to load. Click the "Choose file" button to find one.',
                'No file chosen', self.window)
            return
        ...
        if os.path.splitext(filePath)[1].lower() != '.hps':
            ...
            outputPath = os.path.join(
                globalData.paths['tempFolder'], newFilename)
            ...
            # Convert the file
            command = '"{}" "{}" "{}"{}'.format(
                meleeMediaExe, filePath, outputPath, loopArg)
            returnCode, output = cmdChannel(command)

            if returnCode != 0:
                globalData.gui.updateProgramStatus(
                    'Conversion failed; {}'.format(output))
                msg('File conversion to HPS file format failed; {}'.format(
                    output), 'Conversion Error', self.window)
                return
        ...
        # Initialize the new file
        try:
            self.hpsFile = MusicFile(
                globalData.disc, -1, -1, isoPath, extPath=filePath, source='file')
            self.hpsFile.getData()
        except Exception as err:
            print('Exception during HPS file initialization: ' + str(err))
            msg('A problem occurred during HPS file initialization; {}'.format(
                err), 'Initialization Error', self.window)
            self.hpsFile = None
            return
        ...
        if self.addAsHexTrack.get():
            try:
                ...
                msg('The hex track name includes non-hexadecimal characters or is too large. The number should be between 0 and 0xFF.',
                    'Invalid hex track name', self.window)
                self.hpsFile = None
                return
        ...
        if name:
            try:
                ...
                msg('Unable to set the track name/description; {}'.format(err),
                    'Invalid song name', self.window)
                self.hpsFile = None
                return
...
class AudioEngine(object):
    ...
    def start(self, hpsFileObj, callback=None, playConcurrently=False):
        ...
        assert self.volume >= 0 and self.volume <= 1.0, 'Audio Engine volume is out of range! (currently set to {})'.format(
            self.volume)
        ...
        if not wavFilePath:
            # Indicates an error (should have been conveyed in some way by now)
            return
        ...
        globalData.gui.root.after_idle(
            self.showNowPlaying, hpsFileObj.filename)
        ...
        self.audioThread = Thread(target=self._playAudioHelper, args=(
            wavFilePath, True), name=hpsFileObj.filename)
        # Causes the audio thread to be stopped when the main program stops
    ...
    def showNowPlaying(self, newTrack=''):
        ...
        try:
            ...
            if newTrack:
                isoPath = '{}/audio/{}'.format(
                    globalData.disc.gameId, newTrack)
    ...
    def _playAudioHelper(self, soundFilePath, deleteWav=False):
        ...
        try:
            ...
            assert sampleWidth == 2, 'Unsupported sample width: ' + \
                str(sampleWidth)
...
class AudioControlModule(ttk.Frame, object):
    ...
    def __init__(self, parent, audioEngine, audioFile=None, *args, **kwargs):
        ...
        self.playBtn = ColoredLabelButton(
            self, 'Media Controls/play', self.playAudio, 'Play / Pause')
        self.playBtn.grid(column=0, row=0, padx=spacing)
        self.stopBtn = ColoredLabelButton(
            self, 'Media Controls/stop', self.stop, 'Stop')
        self.stopBtn.grid(column=1, row=0, padx=spacing)
        self.resetBtn = ColoredLabelButton(
            self, 'Media Controls/reset', self.reset, 'Restart')
        self.resetBtn.grid(column=2, row=0, padx=spacing)
        self.repeatBtn = ColoredLabelButton(
            self, 'Media Controls/repeat', self.toggleRepeat, 'Repeat')
        ...
        # Default onvalue/offvalue states for Checkbutton are 1/0, not True/False
        self.repeatAtLoopPoint = Tk.IntVar(value=1)
        repeatCheckbox = ttk.Checkbutton(
            self, text='Repeat at Loop Point', variable=self.repeatAtLoopPoint, command=self.toggleRepeatMode)
        repeatCheckbox.grid(column=0, columnspan=4,
                            row=1, padx=spacing, pady=3)
    ...
    def audioFile(self, audioFile):
        ...
        if self.audioEngine.isPlayingAudio():
            ...
            try:
                ...
                pass  # The button may not exist yet
    ...
    def playAudio(self, event=None):
        ...
        # The above will return immediately; show the pause button while playback happens
        self.showPauseBtn()
    ...
    def toggleRepeat(self, event):
        ...
        if self.audioEngine.playRepeat.isSet():
            ...
            # Set default image to the original highlight color the button was initialized with
            self.repeatBtn.updateColor()
