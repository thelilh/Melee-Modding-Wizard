...
class CharModding(ttk.Notebook):
    ...
    def __init__(self, parent, mainGui):
        ...
        ttk.Label(self.selectionTab, text="Choose the character(s) you'd like to modify:").pack(
            padx=20, pady=20)
    ...
    def checkToAddModeButtons(self):
        ...
        if globalData.disc.is20XX:
            if not self.radioButtonsFrame:
                ...
                ttk.Radiobutton(self.radioButtonsFrame, text='Vanilla', value='.dat',
                                variable=self.extSearch, command=self.populateCharButtons).pack(side='left')
                ttk.Radiobutton(self.radioButtonsFrame, text='SD Remix', value='.sat',
                                variable=self.extSearch, command=self.populateCharButtons).pack(side='left')
                ttk.Radiobutton(self.radioButtonsFrame, text='PAL', value='.pat',
                                variable=self.extSearch, command=self.populateCharButtons).pack(side='left')
    ...
    def populateCharButtons(self):
        ...
        for fileObj in globalData.disc.files.values():
            ...
            if self.texturesInfo:
                ...
                texOffset, _, _, _, texWidth, texHeight, texType, _ = self.texturesInfo[
                    textureIndex]
                icon = self.ifAllFile.getTexture(
                    texOffset, texWidth, texHeight, texType)
            else:
                icon = None

            # Create the button
            button = ttk.Button(self.charBtnsTab, image=icon,
                                text=' ' + fileObj.charName, compound=Tk.LEFT, width=22)
    ...
    def createCharacterTab(self, charFile):
        ...
        if charFile.charAbbr == 'Nn':
            charName = ' Nana'
        elif charFile.charAbbr == 'Pp':
            charName = ' Popo'
        else:
            charName = ' ' + newCharNotebook.charFile.charName
        ...
        newCharNotebook.add(newCharNotebook.subActionEditor,
                            text=' Moves Modding ')
    ...
    def buildPropertiesTab(self, parent):
        ...
        ttk.Label(propertiesTab, text='General Fighter Properties').grid(
            column=0, row=0, pady=12)
        ttk.Label(propertiesTab, text='Special Character Attributes').grid(
            column=1, row=0)
        ...
        for name, formatting, value in zip(propStruct.fields, propStruct.formatting[1:], propertyValues):
            ...
            fieldLabel = ttk.Label(
                structTable.interior, text=propertyName + ':', wraplength=200, justify='center')
            fieldLabel.grid(column=0, row=row, padx=(25, 10),
                            sticky='e', pady=verticalPadding)
            if formatting == 'I':
                typeName = 'Integer'
            else:
                typeName = 'Float'
            ToolTip(fieldLabel, text='Offset in struct: 0x{:X}\nOffset in file: 0x{:X}\nType: {}'.format(
                offset, 0x20 + absoluteFieldOffset, typeName), delay=300)

            # Add an editable field for the raw hex data
            hexEntry = HexEditEntry(structTable.interior, parent.charFile,
                                    absoluteFieldOffset, fieldByteLength, formatting, propertyName, width=11)
            ...
            valueEntry = HexEditEntry(structTable.interior, parent.charFile, absoluteFieldOffset,
                                      fieldByteLength, formatting, propertyName, valueEntry=True, width=15)
            valueEntry.set(value)
            valueEntry.hexEntryWidget = hexEntry
            hexEntry.valueEntryWidget = valueEntry
            valueEntry.grid(column=2, row=row,
                            pady=verticalPadding, padx=(5, 20))
        ...
        for name, formatting, value, note in zip(attrStruct.fields, attrStruct.formatting[1:], propertyValues, attrStruct.notes):
            ...
            if nextSection and nextSection != currentSection:
                ttk.Label(structTable.interior, text=nextSection).grid(
                    columnspan=3, column=0, row=row, padx=(100, 10), pady=(14, 6))
            ...
            toolTipText = 'Offset in struct: 0x{:X}\nOffset in file: 0x{:X}\nType: {}'.format(
                offset, 0x20 + absoluteFieldOffset, typeName)
            if note:
                toolTipText += '\n\n' + note
            ...
            # Add an editable field for the raw hex data
            hexEntry = HexEditEntry(structTable.interior, parent.charFile,
                                    absoluteFieldOffset, 4, formatting, propertyName, width=11)
            rawData = attrStruct.data[offset:offset+4]
            hexEntry.insert(0, hexlify(rawData).upper())
            hexEntry.grid(column=1, row=row, pady=verticalPadding)

            # Add an editable field for this field's actual decoded value (and attach the hex edit widget for later auto-updating)
            valueEntry = HexEditEntry(structTable.interior, parent.charFile,
                                      absoluteFieldOffset, 4, formatting, propertyName, valueEntry=True, width=15)
            valueEntry.set(value)
            valueEntry.hexEntryWidget = hexEntry
            hexEntry.valueEntryWidget = valueEntry
            valueEntry.grid(column=2, row=row,
    ...
    def hasUnsavedChanges(self):
        ...
        for tabName in self.tabs():
            ...
            if not tabWidget.charFile:
                continue  # Skip main selection tab
...
class CharGeneralEditor(ttk.Frame, object):
    ...
    def __init__(self, parent):
        ...
        ColoredLabelButton(cspFrame, 'expandArrowState1',
                           self.prevCostume, 'Previous Costume').grid(column=0, row=1)
        ttk.Button(cspFrame, text='Import').grid(column=1, row=1)
        ttk.Button(cspFrame, text='Export').grid(column=2, row=1)
        ColoredLabelButton(cspFrame, 'expandArrowState2',
                           self.nextCostume, 'Next Costume').grid(column=3, row=1)
...
class ActionEditor(ttk.Frame, object):
    ...
    def __init__(self, parent):
        ...
        ttk.Label(self, textvariable=self.tableTitleVar).grid(
            column=0, columnspan=2, row=0, pady=4)

        filtersBox = ttk.Frame(self)
        ttk.Checkbutton(filtersBox, text='Attacks', variable=globalData.boolSettings['actionStateFilterAttacks']).grid(
            column=0, row=0, sticky='w')
        ttk.Checkbutton(filtersBox, text='Movement', variable=globalData.boolSettings['actionStateFilterMovement']).grid(
            column=0, row=1, sticky='w')
        ttk.Checkbutton(filtersBox, text='Item Related', variable=globalData.boolSettings['actionStateFilterItems']).grid(
            column=1, row=0, sticky='w', padx=(10, 0))
        ttk.Checkbutton(filtersBox, text='Character Specific', variable=globalData.boolSettings['actionStateFilterCharSpecific']).grid(
            column=1, row=1, sticky='w', padx=(10, 0))
        ttk.Checkbutton(filtersBox, text='Empty Entries', variable=globalData.boolSettings['actionStateFilterEmpty']).grid(
        ...
        globalData.boolSettings['actionStateFilterAttacks'].trace(
            'w', self.updateFilters)
        globalData.boolSettings['actionStateFilterMovement'].trace(
            'w', self.updateFilters)
        globalData.boolSettings['actionStateFilterItems'].trace(
            'w', self.updateFilters)
        globalData.boolSettings['actionStateFilterCharSpecific'].trace(
            'w', self.updateFilters)
        globalData.boolSettings['actionStateFilterEmpty'].trace(
            'w', self.updateFilters)
        ...
        ttk.Label(self, text='SubAction Events (a.k.a. ftcmd script):').grid(
            column=2, row=0)
        scrollPane = VerticalScrolledFrame(self)
        self.displayPane = DDList(scrollPane.interior, 500, 40, item_borderwidth=2,
                                  reorder_callback=self.reordered, offset_x=2, offset_y=2, gap=2)
        self.displayPane.pack(fill='both', expand=True)
        scrollPane.grid(column=2, row=1, rowspan=2, sticky='nsew')
        self.displayPaneMessage = ttk.Label(
            self, text='< - Select an Action on\n  the left to begin.', justify='center')
        ...
        generalInfoBox = ttk.Labelframe(
            infoPane, text='  Selected Action  ', padding=(20, 5))
        ...
        ttk.Label(flagsFrame, textvariable=self.subActionFlags).pack(
            side='left', padx=4, pady=(7, 0))
        self.flagsBtn = ColoredLabelButton(
            flagsFrame, 'flag', self.editFlags, 'Edit Flags for this Action')
        ...
        self.eventsFrame = ttk.Labelframe(
            infoPane, text='  SubAction Events  ', padding=(20, 5))
        self.subActionEventsOffset = Tk.StringVar()
        self.subActionEventsSize = Tk.StringVar()
        ttk.Label(self.eventsFrame, textvariable=self.subActionEventsOffset).grid(
            column=0, columnspan=5, row=0)
        ttk.Label(self.eventsFrame, textvariable=self.subActionEventsSize).grid(
            column=0, columnspan=5, row=1)
        ColoredLabelButton(self.eventsFrame, 'delete', self.deleteEvent,
                           'Delete Event', '#f04545').grid(column=0, row=2, pady=4, padx=4)
        ColoredLabelButton(self.eventsFrame, 'expand', self.expandAll, 'Expand All').grid(
            column=1, row=2, pady=4, padx=4)
        ColoredLabelButton(self.eventsFrame, 'collapse', self.collapseAll,
                           'Collapse All').grid(column=2, row=2, pady=4, padx=4)
        ColoredLabelButton(self.eventsFrame, 'save', self.saveEventChanges,
                           'Save Changes\nto Charcter File', '#292').grid(column=3, row=2, pady=4, padx=4)
        insertBtn = LabelButton(self.eventsFrame, 'insert', self.insertEventBefore,
        ...
        ttk.Button(self.eventsFrame, text='Restore to Vanilla', command=self.restoreEvents).grid(
            column=0, columnspan=5, row=3, ipadx=12, pady=4)
        ...
        animBox = ttk.Labelframe(
            infoPane, text='  SubAction Animation  ', padding=(20, 5))
        ...
        ttk.Button(animBox, text='Change Animation',
                   command=self.changeAnimation).pack(ipadx=12, pady=4)
    ...
    def populate(self):
        ...
        # Key = listboxIndex (entry in the widget), value = actionTableEntryIndex
        self.listboxIndices = {}

        title = '{} Action States Table   (0x{:X}):'.format(
            self.charFile.filename, self.actionTable.offset + 0x20)
        ...
        showCharSpecific = globalData.checkSetting(
            'actionStateFilterCharSpecific')
        ...
        for entryIndex, values in self.actionTable.iterateEntries():
            ...
            if namePointer == 0:
                if not showEmpty:
                    ...
                    gameName, friendlyName = self.getActionName(
                        namePointer, entryIndex)
            ...
            if not namePointer:
                self.subActionList.itemconfigure(
                    listboxIndex, foreground='#6A6A6A')
            listboxIndex += 1
    ...
    def getActionName(self, namePointer, index, useParenthesis=False):
        ...
        if namePointer == 0:
            ...
            gameName, friendlyName = self.charFile.getFriendlyActionName(
                None, namePointer, index)
    ...
    def hasUnsavedChanges(self):
        ...
        # Check if the difference is simply due to padding
            ...
            excessData = self.subActionStruct.origData[len(
                self.subActionStruct.data):]
            if any(excessData):  # Returns True if there are any non-zero bytes (meaning it's not padding)
                return True
            else:
                # All the extra data is zeroes (more than likely just padding)
                return False
    ...
    def subActionSelected(self, guiEvent=None, checkForUnsavedChanges=True):
        ...
        # No change!
        if listBoxIndex == self.getListboxFromActionIndex(self.lastSelection):
            return
        ...
        if checkForUnsavedChanges and self.hasUnsavedChanges():
            proceed = tkMessageBox.askyesno(
                'Unsaved Changes Detected', 'It appears there are unsaved changes to the subAction events.\n\nDo you want to discard these changes?')
            if proceed:  # Discard changes
                ...
                self.subActionList.selection_clear(0, 'end')
                self.subActionList.selection_set(
                    self.getListboxFromActionIndex(self.lastSelection))
                return
        ...
        # Convert from listbox index to actions table index
        actionTableIndex = self.listboxIndices[listBoxIndex]
        self.lastSelection = actionTableIndex
        namePointer, animOffset, animSize, eventsPointer, flags, _, _, _ = self.actionTable.getEntryValues(
            actionTableIndex)

        # Update general info display
        self.subActionIndex.set(
            'Table Index:  0x{:X}'.format(actionTableIndex))
        entryOffset = 0x20 + self.actionTable.offset + \
            (actionTableIndex * 0x18)
        ...
        if animOffset == 0:  # Assuming this is supposed to be null/no struct reference, rather than the struct at 0x20
            ...
            self.actionAnimOffset.set(
                'Animation (AJ) Offset:  0x{:X}'.format(0x20+animOffset))
            self.actionAnimSize.set(
                'Animation (AJ) Size:  0x{:X}'.format(animSize))
        ...
        if eventsPointer == 0:  # Assuming this is supposed to be null/no struct reference, rather than the struct at 0x20
            ...
            self.subActionEventsOffset.set(
                'Events Offset:  0x{:X}'.format(0x20+eventsPointer))

            # Get the subAction events structure and parse it
            try:
                self.subActionStruct = self.charFile.initDataBlock(
                    SubAction, eventsPointer, self.actionTable.offset)
                self.subActionStruct.parse()
                self.subActionStruct.origData = self.subActionStruct.data
                self.subActionEventsSize.set(
                    'Events Table Size:  0x{:X}'.format(self.subActionStruct.getLength()))
            except Exception as err:
                self.subActionStruct = None
                actionName = self.getActionName(
                    namePointer, actionTableIndex)[1]
                printStatus('Unable to parse {} subAction (index {}); {}'.format(
                    actionName, actionTableIndex, err))
                self.subActionEventsSize.set('Events Table Size:  N/A')
                return
        ...
        if not self.subActionStruct or (len(self.subActionStruct.events) == 1 and self.subActionStruct.events[0].id == 0):
            if self.displayPaneMessage:
                ...
                self.displayPaneMessage.grid(
                    column=2, row=2, sticky='n', pady=150)
        else:
            if self.displayPaneMessage:
    ...
    def deleteEvent(self, guiEvent):
        ...
        if len(self.displayPane._list_of_items) == 0:
            return
        # Last item is End of Script (not shown in GUI)
        elif self.subActionStruct.events[-1].id == 0:
            assert len(self.displayPane._list_of_items) == len(
                self.subActionStruct.events) - 1, 'Mismatch between self.subActionStruct.events and GUI!'
        else:
            assert len(self.displayPane._list_of_items) == len(
                self.subActionStruct.events), 'Mismatch between self.subActionStruct.events and GUI!'
    ...
    def saveEventChanges(self, guiEvent):
        ...
        if not self.subActionStruct:
            globalData.gui.updateProgramStatus(
                'No subAction events struct has been loaded')
            return
        ...
        if self.subActionStruct.origData == self.subActionStruct.data:
            globalData.gui.updateProgramStatus(
                'No subAction data needs to be saved')
            return
        self.subActionStruct.origData = self.subActionStruct.data
        ...
        actionName = self.getActionName(
            entryValues[0], self.lastSelection, useParenthesis=True)[1]
        self.charFile.updateStruct(self.subActionStruct, 'SubAction event data for {} updated'.format(
            actionName), 'SubAction events updated')

        globalData.gui.updateProgramStatus(
            'These subAction changes have been updated in the character file, but still need to be saved to the disc')
    ...
    def restoreEvents(self):
        ...
        if not vanillaDiscPath:
            printStatus(
                'Unable to restore the file(s) without a vanilla disc to source from', warning=True)
            return
        vanillaDisc = Disc(vanillaDiscPath)
        vanillaDisc.load()

        # Get the vanilla character data file, and get the original events struct from it
        vCharFile = vanillaDisc.getFile(
            'Pl{}.dat'.format(self.charFile.charAbbr))
        vActionTable = vCharFile.getActionTable()
        vEventsPointer = vActionTable.getEntryValues(actionTableIndex)[3]
        vSubActionStruct = vCharFile.initDataBlock(
            SubAction, vEventsPointer, vActionTable.offset)
        ...
        subActionStructSpace = self.charFile.getStructLength(
            self.subActionStruct.offset)  # Might be different than the current struct
        ...
        actionName = self.getActionName(
            namePointer, actionTableIndex, useParenthesis=True)[1]
        description = '{} subAction events restored to vanilla'.format(
            actionName)
        self.charFile.updateStruct(
            self.subActionStruct, description, 'SubAction events restored')
    ...
    def insertEvent(self, index):
        ...
        eM = EventModule(item, window.event,
                         self.displayPane, helpMessage, True)
        eM.pack(fill='both', expand=True)
        item.eventModule = eM  # Useful for the expand/collapse methods below
    ...
    def updateExpansionWarning(self):
        ...
        if newLength > self.subActionStruct.length:
            ...
            if not self.expandInfoBtn:
                self.eventsNotice = ttk.Label(
                    self.eventsFrame, text='Expansion required.', foreground='#a34343')
                self.eventsNotice.grid(column=0, columnspan=4, row=4, pady=4)
                self.expandInfoBtn = LabelButton(
                    self.eventsFrame, 'question', self.showExpansionInfo, 'Details')
    ...
    def changeAnimation(self):
        ...
        selection = self.subActionList.curselection()
        if not selection:
            msg('No action is selected for editing!',
                'Please Select an Action State')
            return
        listBoxIndex = selection[0]
        ...
        # Convert from listbox index to actions table index
        entryIndex = self.listboxIndices[listBoxIndex]
        origNamePointer, ajAnimOffset = self.actionTable.getEntryValues(entryIndex)[
            :2]
        origGameName, origFriendlyName = self.getActionName(
            origNamePointer, entryIndex, useParenthesis=True)

        # Prompt the user to choose an animation
        animSelectWindow = AnimationChooser(
            ajFile, self.charFile, initialSelectionAnimOffset=ajAnimOffset)
        ...
        if animGameName == origGameName and animOffset == ajAnimOffset:
            msg('The action is already using that animation!',
                'Animation Already Applied', warning=True)
            return
        ...
        for _, values in self.actionTable.iterateEntries():
            namePointer = values[0]
            if namePointer != 0:
                # e.g. 'PlyCaptain5K_Share_ACTION_AttackS3S_figatree'
                symbol = self.charFile.getString(namePointer)
            ...
            msg('Unable to find a string referenced in the action table that matches the target animation.',
                'Unable to Swap Animations', error=True)
            printStatus(
                'A matching string from the action table could not be found')
            return
        ...
        message = ('You may enter a new name for this Action/Animation, or leave it as-is. If you do not change this, The action will appear in '
                   ...
                   # todo: add validation to check for existing names
                   'does not already appear in the animation file.')
        newName = getNewNameFromUser(len(
            animGameName), None, message, animGameName, 30, False, 'Enter Action/Animation Name')
        ...
        if newName != animGameName:
            # Results in e.g. ['PlyCaptain5K_Share_ACTION', 'AttackS3S', 'figatree']
            symbolParts = symbol.rsplit('_', 2)
            ...
            for anim in ajFile.animations:
                if anim.offset == animOffset:
                    anim.initialize()
                    anim.name = newSymbol
                    ajStringOffset = 0x20 + animOffset + \
                        anim.headerInfo['stringTableStart']
                    break
            else:  # The loop above didn't break; unable to find the animation
                msg('Unable to find an animation at offset 0x{:X} in the animation file ({})!'.format(
                    animOffset, ajFile.filename), 'Unable to Swap Animations', error=True)
                printStatus('Animation not found in AJ file')
                return

            # Update the strings in both files
            stringChangeDescription = 'Animation name "{}" changed to "{}"'.format(
                origGameName, newName)
            self.charFile.updateData(
                namePointer, symbolData, stringChangeDescription)
            ajFile.updateData(
                ajStringOffset,
                symbolData,
        ...
        if animSelectWindow.friendlyName:
            description = 'Animation for {} changed to {} ({})'.format(
                origFriendlyName, animSelectWindow.friendlyName, animGameName)
        else:
            description = 'Animation for {} changed to {}'.format(
                origFriendlyName, animGameName)

        # Update the action table struct values
        self.charFile.updateStructValue(
            self.actionTable, 0, namePointer, trackChange=False, entryIndex=entryIndex)
        self.charFile.updateStructValue(
            self.actionTable, 1, animOffset, trackChange=False, entryIndex=entryIndex)
        self.charFile.updateStructValue(
            self.actionTable,
            2,
            animSelectWindow.animSize,
            description,
            'Animation swapped',
        ...
        self.actionAnimOffset.set('Animation (AJ) Offset:  0x{:X}'.format(
            0x20+animSelectWindow.animOffset))
        self.actionAnimSize.set(
            'Animation (AJ) Size:  0x{:X}'.format(animSelectWindow.animSize))
    ...
    def editFlags(self, guiEvent):
        ...
        # Convert from listbox index to actions table index
        index = self.listboxIndices[listBoxIndex]
        namePointer = self.actionTable.getEntryValues(index)[0]
        actionName = self.getActionName(
            namePointer, index, useParenthesis=True)[1]
...
class EventModule(ttk.Frame, object):
    ...
    def __init__(self, parentItem, event, displayPane, helpMessage, newItem=False):
        ...
        if newItem:
            ...
            globalData.gui.style.configure(
                'NewEventItem.TFrame', background='#d7f0d7')
            globalData.gui.style.configure(
                'NewEventItem.TLabel', background='#d7f0d7')
        ...
        label = ttk.Label(headerRow, text=self.name, font=(
            'Palatino Linotype', 11, 'bold'), style=self.labelStyle)
        ...
        for field in self.event.fields:
            if field != 'Padding':
                self.expandBtn = ToggleButton(
                    headerRow, 'expandArrow', self.toggleState, style=self.labelStyle)
        ...
        if self.helpMsg:
            helpBtn = LabelButton(headerRow, 'question',
                                  self.showHelp, 'Info', style=self.labelStyle)
...
class EventChooser(BasicWindow):
    ...
    def __init__(self, charFile, index=None):
        ...
        BasicWindow.__init__(self, globalData.gui.root, 'Add Event',
                             resizable=True, minsize=(windowWidth, 600))

        self.charFile = charFile
        self.event = None

        ttk.Label(self.window, text='Choose the type of event to add:').pack(
            pady=12)
        ...
        self.eventsList = Tk.Listbox(
            eventsListFrame,
            yscrollcommand=eventsScrollBar.set,
            activestyle='none',
            selectbackground='#78F'
        )
        eventsScrollBar.config(command=self.eventsList.yview)
        for eventId in range(1, 0x3C):
            eventName = SubAction.eventDesc[eventId][0]
            self.eventsList.insert(eventId, '{}    (0x{:02X} | 0x{:02X})'.format(
                eventName, eventId, eventId * 4))
        ...
        ttk.Label(self.window, textvariable=self.helpText,
                  wraplength=windowWidth-24).pack(padx=10, pady=(14, 0))

        buttonsFrame = ttk.Frame(self.window)
        ttk.Button(buttonsFrame, text='Cancel',
                   command=self.cancel).pack(side='left', padx=20)
        ttk.Button(buttonsFrame, text='Submit',
                   command=self.submit).pack(side='left', padx=20)
        buttonsFrame.pack(expand=False, pady=14)

        # Make this window modal
        # Will not allow the user to interact with main GUI until this is closed
    ...
    def getEventCode(self):
        ...
        if not selection:
            ...
            # Need to add 1 since EoS event is excluded from list
    ...
    def submit(self):
        ...
        self.event = SubActionEvent(
            eventCode, eventDesc[0], length, eventDesc[2], eventDesc[3], eventData)
